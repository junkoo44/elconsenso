...
export default function Home() {
  ...
  const [modalOnlineOpen, setModalOnlineOpen] = useState(false);
  ...
  return (
    <div className="flex-1 w-full max-w-md mx-auto flex flex-col justify-between py-6 px-4 relative">
      ...
      <div className="flex flex-col gap-4 my-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            onClick={() => setModalOnlineOpen(true)}
          >
      ...
      {/* ⚠️ Modal Modo Online (Próximamente) */}
      {modalOnlineOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-space-dark border-2 border-neon-purple rounded-2xl max-w-xs w-full p-6 text-center shadow-2xl relative animate-scale-up">
            <div className="mx-auto mb-4 w-12 h-12 bg-neon-purple/10 border border-neon-purple/30 rounded-xl flex items-center justify-center">
            ...
            <p className="text-text-sub text-[11px] leading-relaxed mb-6 font-semibold">
              El modo multijugador online mediante salas web estará disponible en la versión v2.0. ¡Por ahora jugá en la mesa usando un solo celu!
            </p>
            <button
              type="button"
              className="btn-touch w-full py-3 bg-neon-purple hover:bg-violet-750 text-white font-extrabold rounded-xl text-xs uppercase tracking-widest border-b-4 border-violet-900 cursor-pointer"
              onClick={() => setModalOnlineOpen(false)}
            >
              Entendido
            </button>
          </div>
